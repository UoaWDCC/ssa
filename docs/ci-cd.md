...
Two health checks (`/` for web, `/api/access` for the CMS) run against the machine. A deploy where either app fails to boot is rolled back instead of reporting success and serving 502s.
